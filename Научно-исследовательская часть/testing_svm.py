...
def train_svm_regression(data, vars_dict):
    """
    Обучает модель SVM для регрессии и оценивает ее на тестовой выборке.
    ...
    model = SVR(kernel='rbf', C=6, epsilon=0.2)  # Использование RBF ядра
    ...
    plt.plot(X[min_index:], model.predict(X[min_index:]), 'k--', lw=2, label='Модель')
    plt.xlabel('Признаки')
    plt.ylabel('Целевая переменная')
    plt.title('Сравнение обучающей и тестовой выборок с предсказаниями модели SVM')
    plt.legend()
    plt.grid(True)
    plt.show()
...
results = train_svm_regression(transformed_group, vars_dict)
