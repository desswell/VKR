...
def train_random_forest(data, vars_dict):
    ...
    plt.plot(X[min_index:], rf_model.predict(X[min_index:]), 'k--', lw=2, label='Модель')
    plt.xlabel('Признаки')
...
results = train_random_forest(transformed_group, vars_dict)
